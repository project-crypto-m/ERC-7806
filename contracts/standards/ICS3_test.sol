...
contract ICS3 is IStandard {
    ...
    string public constant DESCRIPTION = "Timed Nonced Pre-Delegated ERC20 Token Swap and Execution Standard";
    ...
    function validateUserIntent(UserIntent calldata intent) external view returns (bytes4) {
        ...
        uint256 numUsedSig = _validateSignatures(intent.sender, unpackedHeader, signedInstructions, intent.signatures);
        if (numUsedSig == 0) {
    ...
    function unpackHeader(bytes calldata header) external pure returns (UnpackedHeader memory) {
        ...
        require(senderHeader.length == 160, "Invalid ICS3 singleton header");
        ...
        return UnpackedHeader(senderHeader, nestedHeader, nonce, timestamp, numOutTokens, numOutTokens + numInstructions, solver);
    }

    function _validateSignatures(address sender, UnpackedHeader memory unpackedHeader, bytes[] memory signedInstructions, bytes[] memory signatures) internal view returns (uint256) {
        ...
        if (unpackedHeader.solver == address(0)) {
            return 0;  // solver is not determined, no need to check nested intent
        }
